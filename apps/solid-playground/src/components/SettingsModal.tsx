            ...
            <div style={{ padding: '16px', "border-bottom": '1px solid var(--app-border)', display: 'flex', "justify-content": 'space-between', "align-items": 'center', background: 'var(--app-bg-panel)' }}>
              ...
              <div style={{ display: 'flex', background: 'var(--app-bg-sidebar)', "border-radius": '6px', overflow: 'hidden', border: '1px solid var(--app-border)' }}>
                 <button 
                  ...
                  style={{ padding: '6px 16px', background: viewMode() === 'rendered' ? 'var(--app-btn-bg)' : 'transparent', color: viewMode() === 'rendered' ? 'var(--app-btn-text)' : 'var(--app-text-muted)', border: 'none', "font-size": '12px', cursor: 'pointer' }}
                >
                  GUI
                </button>
                <button 
                  onClick={() => setViewMode('json')}
                  style={{ padding: '6px 16px', background: viewMode() === 'json' ? 'var(--app-btn-bg)' : 'transparent', color: viewMode() === 'json' ? 'var(--app-btn-text)' : 'var(--app-text-muted)', border: 'none', "font-size": '12px', cursor: 'pointer' }}
                >
                  JSON
                </button>
              </div>
